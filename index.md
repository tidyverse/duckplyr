...
Calling [`library(duckplyr)`](https://duckplyr.tidyverse.org) overwrites
...
dataset, where the timezone has been set to UTC to work around a current
limitation of duckplyr, see
...
Note that, unlike dplyr, the results are not ordered, see
[`?config`](https://duckplyr.tidyverse.org/reference/config.md) for
...
Restart R, or call
[`duckplyr::methods_restore()`](https://duckplyr.tidyverse.org/reference/methods_overwrite.md)
...
with a [`collect()`](https://dplyr.tidyverse.org/reference/compute.html)
call for instance.
