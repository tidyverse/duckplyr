...
func_decl <- function(name, formals) {
  ...
  sym_formals <- rlang::set_names(
    rlang::syms(names(formals)),
    names(formals)
  )
...
func_decl_chr <- function(
  ...
  rel_try_chr <- paste0(
    ...
    '    "No relational implementation for {.code ',
    generic,
    '()}" = TRUE,\n',
    "    {\n",
    "      return(out)\n",
  ...
  dplyr_code <- brio::read_file(fs::path(
    "dplyr-methods",
    paste0(generic, ".txt")
  ))
  dplyr_impl <- c(
  ...
  test_code <- whisker::whisker.render(
    test_code
  )
