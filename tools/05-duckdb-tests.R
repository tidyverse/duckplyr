...
get_duckdb_test_code <- function(
  ...
  two_tables <- (length(formals) > 1) &&
    (names(formals)[[2]] == "y")

  extra_args <- test_extra_arg_map[[
    name
  ]] %||%
    c("")

  if (length(extra_args) == 1) {
    with_force <- test_force_override[
      name
    ] %|%
  ...
  paste0(
    ...
    desc,
    " ",
    strrep("-", 85 - nchar(desc)),
    "\n\n",
    out
  )
}

get_duckdb_test_code_one <- function(
  extra_arg,
  pre_step,
  oo,
  name,
  ...
  test_fun_code <- whisker::whisker.render(
    test_fun_code
  )
  test_fun <- eval(parse(
    text = test_fun_code
  ...
  meta_code <- utils::capture.output(meta_replay(
    add_pre_code = TRUE
  ))
  meta_code[[length(
    meta_code
  ...
  meta_code <- c(
    ...
    paste0(
      "  ",
      utils::capture.output(constructive::construct(
        post
      ))
    ),
    ")",
  ...
  test_code <- whisker::whisker.render(c(
    test_code_pre,
    test_code,
    test_code_post
  ))
...
tests <-
  ...
  mutate(
    sort = lengths(test_extra_arg_map[
      name
    ])
  ) %>%
  arrange(desc(sort)) %>%
