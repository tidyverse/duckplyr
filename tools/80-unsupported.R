check_r_script <- function(path) {
  ...
  not_implemented <- any(grepl(
    "No relational implementation for |instead of ..code group_by... and ..code ungroup...",
    lines
  ))
...
not_implemented <- unlist(lapply(
  r_scripts,
  check_r_script
))

not_implemented_script <- file.path(
  "R",
  "not-supported.R"
)

unlink(
  not_implemented_script,
  force = TRUE
)
