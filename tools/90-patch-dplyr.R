...
stopifnot(all(
  forbidden %in% fs::dir_ls("R")
))

duckplyr_files <- setdiff(
  fs::dir_ls("R"),
  forbidden
)
duckplyr_texts <- map_chr(
  duckplyr_files,
  brio::read_file
)
duckplyr_texts <- str_replace_all(
  duckplyr_texts,
  "dplyr[$]([a-z_]+)[.]data[.]frame",
  "\\1_data_frame"
)
duckplyr_texts <- str_replace_all(
  duckplyr_texts,
  fixed(".duckplyr_df <- function("),
  ".data.frame <- function("
)
duckplyr_texts <- str_replace(
  duckplyr_texts,
  "^",
  paste0(
    "# begin ",
...
brio::write_lines(
  duckplyr_texts,
  ".sync/dplyr-revdep/R/zzz-duckplyr.R"
)
...
patch_dplyr_test <- function(file) {
  ...
  skip_todo <- unlist(all_tests[
    names(all_tests) == base
  ])
...
dplyr_test_files <- fs::dir_ls(
  ".sync/dplyr-revdep/tests/testthat/",
  type = "file",
  glob = "*.R"
)
...
message <- paste0(
  "Sync with tidyverse/duckplyr@",
  gert::git_log(max = 1)$commit
)
gert::git_commit(
  message,
  repo = ".sync/dplyr-revdep"
