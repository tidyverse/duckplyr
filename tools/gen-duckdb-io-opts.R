...
get_write_opts <- function(con, format, candidates) {
  ...
  for (opt in candidates) {
    ...
    sql <- paste0(
      "COPY write_test_tbl TO '", tempfile(),
      "' (FORMAT ", format, ", ", opt, " ", val, ")"
    )
    tryCatch(
      {
        dbExecute(con, sql)
...
parquet_candidates <- c(
  "append", "binary_as_string", "can_have_nan", "codec", "compression",
  "compression_level", "debug_use_openssl", "dictionary_size_limit",
  "encryption_config", "explicit_cardinality", "field_ids", "file_extension",
  "file_row_number", "file_size_bytes", "filename", "filename_pattern",
  "hive_file_pattern", "hive_partitioning", "hive_types", "hive_types_autocast",
  "kv_metadata", "overwrite", "overwrite_or_ignore", "parquet_version",
  "partition_by", "per_thread_output", "preserve_order", "return_files",
  "return_stats", "row_group_size", "row_group_size_bytes", "schema",
  "union_by_name", "use_tmp_file", "write_partition_columns"
)

csv_candidates <- c(
  "all_varchar", "allow_quoted_nulls", "append", "auto_detect",
  "auto_type_candidates", "buffer_size", "column_names", "column_types",
  "columns", "comment", "compression", "dateformat", "decimal_separator",
  "delim", "dtypes", "encoding", "escape", "file_extension",
  "file_size_bytes", "filename", "filename_pattern", "files_to_sniff",
  "force_not_null", "force_quote", "header", "hive_file_pattern",
  "hive_partitioning", "hive_types", "hive_types_autocast", "ignore_errors",
  "max_line_size", "maximum_line_size", "names", "new_line", "normalize_names",
  "null_padding", "nullstr", "overwrite", "overwrite_or_ignore",
  "parallel", "partition_by", "per_thread_output", "prefix", "preserve_order",
  "quote", "rejects_limit", "rejects_scan", "rejects_table", "return_files",
  "return_stats", "sample_size", "sep", "skip", "store_rejects",
  "strict_mode", "suffix", "thousands", "timestampformat", "types",
  "union_by_name", "use_tmp_file", "write_partition_columns"
)
