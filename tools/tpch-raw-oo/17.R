...
rel15 <- duckdb$rel_set_alias(rel10, "lhs")
...
rel21 <- duckdb$rel_join(
  ...
  list(
    duckdb$expr_function(
      "==",
      list(duckdb$expr_reference("l_partkey_x", rel19), duckdb$expr_reference("l_partkey_y", rel20))
    )
  ),
  "inner"
)
...
rel26 <- duckdb$rel_distinct(rel25)
rel26
