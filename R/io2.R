...
#' @export
read_parquet_duckdb <- function(path, ..., collect = c(cells = 1e6), options = list()) {
  check_dots_empty()
...
#' )
read_csv_duckdb <- function(path, ..., collect = c(cells = 1e6), options = list()) {
  check_dots_empty()
...
read_json_duckdb <- function(path, ..., collect = c(cells = 1e6), options = list()) {
  check_dots_empty()
...
#' See the "Prudence" section in [duckdb_tibble()] for details.
#'
...
#'   By default, a frugal duckplyr frame is created.
#'   See the "Prudence" section in [duckdb_tibble()] for details.
