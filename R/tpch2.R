...
tpch_11 <- function() {
  ...
  global_agr <- joined_filtered %>%
    summarise(
      global_value = sum(na.rm = TRUE, ps_supplycost * ps_availqty) *
        0.0001000000
    ) %>%
...
tpch_20 <- function() {
  ...
  qty_threshold <- lineitem %>%
    ...
    semi_join(
      na_matches = TPCH_NA_MATCHES,
      partsupp_forest_ca,
      by = c("l_partkey" = "ps_partkey", "l_suppkey" = "ps_suppkey")
    ) %>%
    summarise(
      qty_threshold = 0.5 * sum(na.rm = TRUE, l_quantity),
      .by = l_suppkey
    )
  ...
  supplier_ca %>%
    semi_join(
      na_matches = TPCH_NA_MATCHES,
      partsupp_forest_ca_filtered,
      by = c("s_suppkey" = "ps_suppkey")
    ) %>%
...
tpch_22 <- function() {
  acctbal_mins <- customer %>%
    filter(
      ...
      substr(c_phone, 1L, 2L) %in%
        c("13", "31", "23", "29", "30", "18", "17") &
        c_acctbal > 0
    ) %>%
