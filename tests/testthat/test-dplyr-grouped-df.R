...
test_that("new_grouped_df can create alternative grouping structures (#3837)", {
  ...
  res <- duckplyr_summarise(tbl, x = mean(x))
...
test_that("[ method can remove grouping vars", {
  ...
  expect_equal(gf, gf)
  expect_equal(gf[1], duckplyr_group_by(df[1], x))
...
test_that("can remove grouping cols with subset assignment", {
  df <- tibble(x = 1, y = 2)
  gf1 <- gf2 <- gf3 <- duckplyr_group_by(df, x, y)
...
test_that("names<- doesn't modify group data if not necessary", {
  df <- tibble(x = 1, y = 2)
  gf1 <- gf2 <- duckplyr_group_by(df, x)
...
test_that("group order is maintained in grouped-df methods (#5040)", {
  gdf <- duckplyr_group_by(mtcars, cyl, am, vs)

  x <- gdf[0, ]
  expect_identical(duckplyr_group_vars(x), duckplyr_group_vars(gdf))
  ...
  names(x) <- names
  expect_identical(
    duckplyr_group_vars(x),
    duckplyr_group_vars(duckplyr_group_by(x, cyl, am2, vs))
  )
})
...
test_that("validate_grouped_df() gives useful errors", {
  df1 <- duckplyr_group_by(tibble(x = 1:4, g = rep(1:2, each = 2)), g)
  groups <- attr(df1, "groups")
  groups[[2]] <- 1:2
  attr(df1, "groups") <- groups

  df2 <- duckplyr_group_by(tibble(x = 1:4, g = rep(1:2, each = 2)), g)
...
test_that("using the deprecated global option `dplyr.legacy_locale` forces the system locale", {
  ...
  # Confirming that the deprecation warning also shows up in `duckplyr_group_by()` itself
  expect_snapshot({
    result <- duckplyr_group_by(df, x)
  })
