...
test_that('.funnel = "always_manual" forbids materialization', {
  tbl <- duckdb_tibble(a = 1, .funnel = "always_manual")
...
test_that('.funnel = "always_manual" forbids materialization for as_duckdb_tibble', {
  tbl <- as_duckdb_tibble(data.frame(a = 1), funnel = "always_manual")
  expect_error(length(tbl$a))
})
...
test_that("as_duckdb_tibble() and dbplyr tables", {
  ...
  duck <- db_tbl %>%
    as_duckdb_tibble(funnel = "always_manual") %>%
    mutate(b = 2)
