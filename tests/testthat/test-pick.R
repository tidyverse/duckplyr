...
test_that("can pick columns from the data", {
  ...
  out <- duckplyr_mutate(df, sel = pick(z, starts_with("x")))
  expect_identical(out$sel, expect)

  out <- duckplyr_mutate(df, sel = pick_wrapper(z, starts_with("x")))
  expect_identical(out$sel, expect)
})
...
test_that("can use namespaced call to `pick()`", {
  ...
  expect_identical(
    duckplyr_mutate(df, z = dplyr::pick(where(is.character))),
    duckplyr_mutate(df, z = pick(where(is.character)))
  )
})
...
test_that("returns separate data frames for each group", {
  ...
  expect <- duckplyr_mutate(gdf, res = x + mean(z))

  out <- duckplyr_mutate(gdf, res = fn(pick(x, z)))
  expect_identical(out, expect)

  out <- duckplyr_mutate(gdf, res = fn(pick_wrapper(x, z)))
  expect_identical(out, expect)
})
...
test_that("returns a tibble", {
  ...
  out <- duckplyr_mutate(df, y = pick(x))
  expect_s3_class(out$y, "tbl_df")

  out <- duckplyr_mutate(df, y = pick_wrapper(x))
  expect_s3_class(out$y, "tbl_df")
})

test_that("with `duckplyr_rowwise()` data, leaves list-cols unwrapped (#5951, #6264)", {
  ...
  expect_snapshot(error = TRUE, {
    duckplyr_mutate(rdf, z = pick(x, y))
  })
  expect_snapshot(error = TRUE, {
    duckplyr_mutate(rdf, z = pick_wrapper(x, y))
  })
})
...
test_that("selectors won't select grouping columns", {
  ...
  out <- duckplyr_mutate(gdf, y = pick(everything()))
  expect_named(out$y, "x")

  out <- duckplyr_mutate(gdf, y = pick_wrapper(everything()))
  expect_named(out$y, "x")
})
...
test_that("selectors won't select rowwise 'grouping' columns", {
  ...
  out <- duckplyr_mutate(rdf, y = pick(everything()))
  expect_named(out$y, "x")

  out <- duckplyr_mutate(rdf, y = pick_wrapper(everything()))
  expect_named(out$y, "x")
})
...
test_that("can't explicitly select grouping columns (#5460)", {
  ...
  expect_snapshot(error = TRUE, {
    duckplyr_mutate(gdf, y = pick(g))
  })
  expect_snapshot(error = TRUE, {
    duckplyr_mutate(gdf, y = pick_wrapper(g))
  })
})
...
test_that("`all_of()` is evaluated in the correct environment (#5460)", {
  ...
  expect_error(duckplyr_mutate(df, z = pick(all_of(y))))
  expect_error(duckplyr_mutate(df, z = pick_wrapper(all_of(y))))

  y <- "x"
  expect <- df["x"]

  out <- duckplyr_mutate(df, z = pick(all_of(y)))
  expect_identical(out$z, expect)

  out <- duckplyr_mutate(df, z = pick_wrapper(all_of(y)))
  expect_identical(out$z, expect)
})
...
test_that("empty selections create 1 row tibbles (#6685)", {
  ...
  # a `pick(NULL)` call can be used in a `duckplyr_group_by()` wrapper to
  ...
  out <- duckplyr_mutate(gdf, y = pick(starts_with("foo")))
  expect_identical(out$y, new_tibble(list(), nrow = 3L))

  out <- duckplyr_mutate(gdf, y = pick_wrapper(starts_with("foo")))
  expect_identical(out$y, new_tibble(list(), nrow = 3L))
})
...
test_that("the tidyselection and column extraction are evaluated on the current data", {
  ...
  expect_snapshot(error = TRUE, {
    ...
    duckplyr_mutate(gdf, x = NULL, y = pick(x))
  })
  expect_snapshot(error = TRUE, {
    # Does actual `eval_select()` call per group
    duckplyr_mutate(gdf, x = NULL, y = pick_wrapper(x))
  })

  # Can select newly created columns
  out <- duckplyr_mutate(gdf, y = x + 1L, z = pick(x, y))
  expect_identical(out[c("x", "y")], out$z)

  out <- duckplyr_mutate(gdf, y = x + 1L, z = pick_wrapper(x, y))
  expect_identical(out[c("x", "y")], out$z)
  ...
  out <- duckplyr_mutate(df, x = pick(x), x = pick(x), y = pick(x))
  expect_identical(out, expect)

  out <- duckplyr_mutate(df, x = pick_wrapper(x), x = pick_wrapper(x), y = pick_wrapper(x))
  expect_identical(out, expect)
})
...
test_that("can call different `pick()` expressions in different groups", {
  ...
  out <- duckplyr_mutate(gdf, z = if (g == 1) pick(x) else pick(y))
  expect_identical(out$z, expect)

  out <- duckplyr_mutate(gdf, z = if (g == 1) pick_wrapper(x) else pick_wrapper(y))
  expect_identical(out$z, expect)
})

test_that("can call `pick()` from a user defined function", {
  df <- tibble(a = 1, b = 2, c = 3)
  gdf <- duckplyr_group_by(df, a)

  # Hardcoded variables in expression
  my_pick <- function() pick(a, c)
  out <- duckplyr_mutate(df, d = my_pick())
  ...
  y <- c("a", "c")
  out <- duckplyr_mutate(df, d = my_pick(y))
  expect_identical(out$d, df[c("a", "c")])

  expect_snapshot(error = TRUE, {
    duckplyr_mutate(gdf, d = my_pick(y))
  })
})
...
test_that("wrapped `all_of()` and `where()` selections work", {
  ...
  out <- duckplyr_mutate(df, x = my_pick("a"), y = my_pick("b"))
  ...
  out <- duckplyr_mutate(df, x = my_pick("a"), y = my_pick2("b"))
  expect_identical(out$x, df["a"])
  expect_identical(out$y, df["b"])
  ...
  out <- duckplyr_mutate(df, x = my_where(is.numeric), y = my_where(is.character))
...
test_that("`pick()` expansion evaluates on the full data", {
  ...
  out <- duckplyr_mutate(gdf, y = pick(where(~all(.x == 0))))
  ...
  out <- duckplyr_mutate(gdf, z = pick_wrapper(where(~all(.x == 0))))
...
test_that("can assign `pick()` to new function", {
  ...
  out <- duckplyr_mutate(df, z = pick2(y))
...
test_that("selection on rowwise data frames uses full list-cols, but actual evaluation unwraps them", {
  ...
  out <- duckplyr_mutate(df, y = list(pick(where(is.list))))
...
# ------------------------------------------------------------------------------
# pick() + duckplyr_summarise()/duckplyr_reframe()

test_that("can `pick()` inside `duckplyr_reframe()`", {
  ...
  out <- duckplyr_reframe(df, vec_count(pick(x, y), sort = "count"))
  expect_identical(out$key, expect_key)
  expect_identical(out$count, expect_count)

  out <- duckplyr_reframe(df, vec_count(pick_wrapper(x, y), sort = "count"))
  expect_identical(out$key, expect_key)
  expect_identical(out$count, expect_count)
  ...
  out <- duckplyr_reframe(gdf, vec_count(pick(x, y), sort = "count"))
  expect_identical(out$key, expect_key)
  expect_identical(out$count, expect_count)

  out <- duckplyr_reframe(gdf, vec_count(pick_wrapper(x, y), sort = "count"))
  expect_identical(out$key, expect_key)
  expect_identical(out$count, expect_count)
})
...
test_that("empty selections recycle to the size of any other column", {
  ...
  out <- duckplyr_summarise(df, sum = sum(x), y = pick(starts_with("foo")))
  expect_identical(out$sum, 15L)
  expect_identical(out$y, new_tibble(list(), nrow = 1L))

  out <- duckplyr_summarise(df, sum = sum(x), y = pick_wrapper(starts_with("foo")))
  expect_identical(out$sum, 15L)
  expect_identical(out$y, new_tibble(list(), nrow = 1L))

  # Returns size 1 tibbles that recycle to size 0 because of `empty` (#6685)
  out <- duckplyr_reframe(df, empty = integer(), y = pick(starts_with("foo")))
  expect_identical(out$empty, integer())
  expect_identical(out$y, new_tibble(list(), nrow = 0L))

  out <- duckplyr_reframe(df, empty = integer(), y = pick_wrapper(starts_with("foo")))
  expect_identical(out$empty, integer())
  expect_identical(out$y, new_tibble(list(), nrow = 0L))
})

test_that("uses 'current' columns of `summarize()` and `duckplyr_reframe()`", {
  ...
  out <- duckplyr_summarise(df, x = sum(x), z = pick(x))
  expect_identical(out$x, expect_x)
  expect_identical(out$z, expect_z)

  out <- duckplyr_summarise(df, x = sum(x), z = pick_wrapper(x))
  expect_identical(out$x, expect_x)
  expect_identical(out$z, expect_z)
  ...
  out <- duckplyr_reframe(df, x = sum(x), z = pick(x, y))
  expect_identical(out$x, expect_x)
  expect_identical(out$z, expect_z)

  out <- duckplyr_reframe(df, x = sum(x), z = pick_wrapper(x, y))
  expect_identical(out$x, expect_x)
  expect_identical(out$z, expect_z)
})

test_that("can select completely new columns in `duckplyr_summarise()`", {
  df <- tibble(x = 1:5)

  out <- duckplyr_mutate(df, y = x + 1, z = pick(y))
  expect_identical(out["y"], out$z)

  out <- duckplyr_mutate(df, y = x + 1, z = pick_wrapper(y))
  expect_identical(out["y"], out$z)
})

# ------------------------------------------------------------------------------
# pick() + duckplyr_arrange()

test_that("can `duckplyr_arrange()` with `pick()` selection", {
  df <- tibble(x = c(2, 2, 1), y = c(3, 1, 3))

  expect <- df[c(3, 2, 1),]

  expect_identical(duckplyr_arrange(df, pick(x, y)), expect)
  expect_identical(duckplyr_arrange(df, pick_wrapper(x, y)), expect)

  expect_identical(duckplyr_arrange(df, pick(x), y), expect)
  expect_identical(duckplyr_arrange(df, pick_wrapper(x), y), expect)
})

test_that("`pick()` errors in `duckplyr_arrange()` are useful", {
  df <- tibble(x = 1)
...
# ------------------------------------------------------------------------------
# pick() + duckplyr_group_by()

test_that("`pick()` can be used inside `duckplyr_group_by()` wrappers", {
  df <- tibble(a = 1:3, b = 2:4, c = 3:5)

  tidyselect_group_by <- function(data, groups) {
    duckplyr_group_by(data, pick({{ groups }}))
  }
  tidyselect_group_by_wrapper <- function(data, groups) {
    duckplyr_group_by(data, pick_wrapper({{ groups }}))
  }

  expect_identical(
    tidyselect_group_by(df, c(a, c)),
    duckplyr_group_by(df, a, c)
  )
  expect_identical(
