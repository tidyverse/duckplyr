...
test_that("slice does not evaluate the expression in empty groups (#1438)", {
  res <- mtcars %>%
    ...
    duckplyr_filter(cyl==6) %>%
    duckplyr_slice(1:2)
  expect_equal(nrow(res), 2L)

  expect_error(
    res <- mtcars %>% duckplyr_group_by(cyl) %>% duckplyr_filter(cyl==6) %>% sample_n(size=3),
    NA
  )
...
test_that("sampling grouped tbl samples each group", {
  sampled <- mtcars %>% duckplyr_group_by(cyl) %>% sample_n(2)
  expect_s3_class(sampled, "grouped_df")
  expect_equal(duckplyr_group_vars(sampled), "cyl")
...
test_that("sample_n and sample_frac handles lazy grouped data frames (#3380)", {
  ...
  res <- df1 %>% duckplyr_group_by(y) %>% duckplyr_anti_join(df2, by="x") %>% sample_n(1)
  expect_equal(nrow(res), 1L)

  res <- df1 %>% duckplyr_group_by(y) %>% duckplyr_anti_join(df2, by="x") %>% sample_frac(0.2)
  expect_equal(nrow(res), 1L)
})
...
test_that("sample_*() gives meaningful error messages", {
  expect_snapshot({
    ...
    (expect_error(
      mtcars %>% duckplyr_group_by(cyl) %>% sample_n(10)
    ))
    ...
    (expect_error(
      sample_frac(df %>% duckplyr_group_by(y), 2)
    ))
    (expect_error(
