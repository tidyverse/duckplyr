...
test_that("select_if() and rename_if() handles logical (#4213)", {
  ...
  expect_identical(
    ...
    iris %>% duckplyr_select(-Sepal.Length)
  )

  expect_identical(
    iris %>% rename_if(!names(.) %in% ids, toupper),
    iris %>% rename_at(duckplyr_setdiff(names(.), "Sepal.Length"), toupper)
  )
...
test_that("colwise duckplyr_select() / duckplyr_rename() give meaningful errors", {
  expect_snapshot({
    df <- tibble(x = 0L, y = 0.5, z = 1)

    # colwise duckplyr_rename()
    (expect_error(
      df %>% rename_all()
    ))
    (expect_error(
