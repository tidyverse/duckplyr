...
test_that("across() does not select grouping variables", {
  ...
  out <- df |>
    duckplyr_group_by(g) |>
    duckplyr_summarise(x = across(everything(), identity)) |>
    duckplyr_pull()
...
test_that("across() correctly names output columns", {
  ...
  expect_named(
    duckplyr_summarise(
      gf,
      across(everything(), identity, .names = "id_{.col}")
    ),
    c("x", "id_y", "id_z", "id_s")
  )
  expect_named(
  ...
  expect_named(
    duckplyr_summarise(
      gf,
      across(where(is.numeric), list(mean = mean, sum = sum))
    ),
...
test_that("across(.unpack =) can unpack data frame columns", {
  ...
  out <- duckplyr_mutate(
    df,
    across(x:y, list(one = fn1, two = fn2), .unpack = TRUE)
  )
...
test_that("across(.unpack =) skips unpacking non-df-cols", {
  ...
  out <- duckplyr_mutate(
    df,
    across(x, list(fn = fn, double = ~ .x * 2), .unpack = TRUE)
  )
...
test_that("across() result locations are aligned with column names (#4967)", {
  ...
  x <- duckplyr_summarise(
    df,
    across(everything(), list(cls = class, type = is.numeric))
  )
...
test_that("across() retains original ordering", {
  ...
  df <- tibble(a = 1, b = 2)
  expect_named(
    duckplyr_mutate(df, a = 2, x = across(everything(), identity))$x,
    c("a", "b")
  )
})
...
test_that("across() uses tidy recycling rules", {
  ...
  expect_equal(
    data.frame(x = 1, y = 2) |>
      duckplyr_reframe(across(everything(), ~ rep(42, .))),
...
test_that("across(.unpack=) can use local variables in addition to {outer} and {inner}", {
  ...
  res <- local({
    ...
    data.frame(col1 = 42, col2 = 24) |>
      duckplyr_summarise(across(
        everything(),
        fn,
        .unpack = "{prefix}_{outer}_{inner}"
      ))
  })
...
test_that("across() uses environment from the current quosure (#5460)", {
  ...
  expect_equal(
    df |>
      duckplyr_summarise(duckplyr_summarise(
        pick(everything()),
        across(all_of(y), mean)
      )),
    df |> duckplyr_summarise(across(all_of(y), mean))
  )
})
...
test_that("lambdas in duckplyr_summarise() + across() can use columns in follow up expressions (#5717)", {
  ...
  expect_identical(
    df |>
      duckplyr_summarise(a = 2, data.frame(x = x / y, y = y / y, z = z / y)),
...
test_that("inlined and non inlined lambdas work", {
  ...
  expect_equal(
    df |> duckplyr_mutate(across(1:2, function(x) x + mean(bar))),
    exp
  )
  expect_equal(
    df |> duckplyr_mutate((across(1:2, function(x) x + mean(bar)))),
...
test_that("list of lambdas work", {
  ...
  expect_equal(
    df |> duckplyr_mutate(across(1:2, list(function(x) x + mean(bar)))),
    exp
  )
  expect_equal(
    df |> duckplyr_mutate((across(1:2, list(function(x) x + mean(bar))))),
  ...
  expect_equal(
    df |> duckplyr_mutate((across(1:2, list(~ .x + mean(bar))))),
    exp
  )
})
...
test_that("across() allows renaming in `.cols` (#6895)", {
  ...
  expect_identical(
    duckplyr_mutate(
      df,
      (across(all_of(cols), identity, .names = "{.col}_name"))
    ),
    duckplyr_mutate(df, a_name = x, b_name = y)
  )
})
...
test_that("across() can use named selections", {
  ...
  expect_equal(
    df |>
      duckplyr_summarise(across(c(a = x, b = y), list(mean = mean, sum = sum))),
  ...
  expect_equal(
    df |>
      duckplyr_summarise(
        (across(c(a = x, b = y), list(mean = mean, sum = sum)))
      ),
    data.frame(a_mean = 1, a_sum = 1, b_mean = 2, b_sum = 2)
  )
  expect_equal(
