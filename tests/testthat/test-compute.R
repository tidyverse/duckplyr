test_that("compute()", {
  ...
  expect_false(is_inert_duckplyr_df(out))
})

test_that("inerting with failure", {
  set.seed(20250124)
...
test_that("inerting with success", {
  set.seed(20250126)

  df <- duckdb_tibble(x = 1:10, .inert = c(rows = 20))
  out <- compute(df)

  expect_identical(collect(out), collect(df))
  expect_identical(get_inert_duckplyr_df(out), c(rows = 20))
