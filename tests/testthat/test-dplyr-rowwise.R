...
test_that("rowwise status preserved by major verbs", {
  rf <- duckplyr_rowwise(tibble(x = 1:5, y = 5:1), "x")

  out <- duckplyr_arrange(rf, y)
  expect_s3_class(out, "rowwise_df")
  expect_equal(duckplyr_group_vars(out), "x")

  out <- duckplyr_filter(rf, x < 3)
  expect_s3_class(out, "rowwise_df")
  expect_equal(duckplyr_group_vars(out), "x")
  ...
  out <- duckplyr_rename(rf, X = x)
  expect_s3_class(out, "rowwise_df")
  expect_equal(duckplyr_group_vars(out), "X")

  out <- duckplyr_select(rf, "x")
  expect_s3_class(out, "rowwise_df")
  expect_equal(duckplyr_group_vars(out), "x")

  out <- duckplyr_slice(rf, c(1, 1))
  expect_s3_class(out, "rowwise_df")
  expect_equal(duckplyr_group_vars(out), "x")

  # Except for summarise
  out <- duckplyr_summarise(rf, z = mean(x, y))
  expect_s3_class(out, "grouped_df")
  expect_equal(duckplyr_group_vars(out), "x")
})

test_that("rowwise nature preserved by subsetting ops", {
  rf <- duckplyr_rowwise(tibble(x = 1:5, y = 1:5), "x")
...
test_that("rowwise has decent print method", {
  rf <- duckplyr_rowwise(tibble(x = 1:5), "x")
  expect_snapshot(rf)
})

test_that("rowwise captures group_vars", {
  df <- duckplyr_group_by(tibble(g = 1:2, x = 1:2), g)
  rw <- duckplyr_rowwise(df)
  expect_equal(duckplyr_group_vars(rw), "g")

  # but can't regroup
  expect_error(duckplyr_rowwise(df, x), "Can't re-group")
})

test_that("can re-rowwise", {
  rf1 <- duckplyr_rowwise(tibble(x = 1:5, y = 1:5), "x")
  rf2 <- duckplyr_rowwise(rf1, y)
  expect_equal(duckplyr_group_vars(rf2), "y")
})
...
test_that("validate_rowwise_df() gives useful errors", {
  df1 <- duckplyr_rowwise(tibble(x = 1:4, g = rep(1:2, each = 2)), g)
  groups <- attr(df1, "groups")
  groups[[2]] <- 4:1
  attr(df1, "groups") <- groups

  df2 <- duckplyr_rowwise(tibble(x = 1:4, g = rep(1:2, each = 2)), g)
  ...
  df7 <- duckplyr_rowwise(tibble(x = 1:10))
  attr(df7, "groups")$.rows <- 11:20

  df8 <- duckplyr_rowwise(tibble(x = 1:10))
