...
test_that("zero-arg row_number() works in mutate", {
  ...
  expect_equal(duckplyr_mutate(df, rn = row_number())$rn, 1:sum(n))

  gf <- duckplyr_group_by(df, id)
  expect_equal(duckplyr_mutate(gf, rn = row_number())$rn, sequence(n))
})
...
test_that("ntile() works with one argument (#3418)", {
  ...
  expect_equal(duckplyr_mutate(df, out = ntile(n = 3))$out, c(1, 1, 2, 2, 3))

  gf <- duckplyr_group_by(df, id)
  expect_equal(duckplyr_mutate(gf, out = ntile(n = 2))$out, c(1, 2, 1, 1, 2))
})
