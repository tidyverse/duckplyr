...
```{r error = TRUE}
duckplyr::duckdb_tibble()
duckplyr::duckdb_tibble(a = 1, .prudence = "frugal") |>
  select(-a)
```
...
```{r}
duckplyr::duckdb_tibble(a = 1, b = 2, c = 3, .prudence = "frugal") |>
  mutate((a + b) * c)
```
...
```{r}
duckplyr::duckdb_tibble(a = FALSE, b = TRUE, c = NA, .prudence = "frugal") |>
  mutate(!a, a & b, b | c)
```
...
```{r}
duckplyr::duckdb_tibble(a = 1, b = NA, .prudence = "frugal") |>
...
```{r}
duckplyr::duckdb_tibble(a = "abbc", .prudence = "frugal") |>
...
```{r}
duckplyr::duckdb_tibble(a = 1:3, b = c(1, 2, 2), .prudence = "frugal") |>
  summarize(
    sum(a), 
    n(),
    n_distinct(b), # na.rm currently not supported, defaults to TRUE
  )

duckplyr::duckdb_tibble(a = 1:3, b = c(1, 2, NA), .prudence = "frugal") |>
  summarize(
    mean(b, na.rm = TRUE), 
    median(a),
    sd(b),
...
duckplyr::duckdb_tibble(a = 1:3, .prudence = "frugal") |>
  summarize(
    min(a), # min(FALSE) returns FALSE here, not 0
    max(a), # max(FALSE) returns FALSE here, not 0
    any(a > 1), 
    all(a > 1),
  )
```
...
duckplyr::duckdb_tibble(a = 1:3, b = c(2, 3, 1), .prudence = "frugal") |>
...
Implementing `dplyr::ntile(n)` is possible.
The only ranking function currently implemented is `dplyr::row_number()`.

```{r}
duckplyr::duckdb_tibble(a = c(1, 2, 2, 3), .prudence = "frugal") |>
  mutate(row_number())
```
...
```{r}
b <- 4
duckplyr::duckdb_tibble(a = 1, b = 2, .prudence = "frugal") |>
...
### Support for more functions needed?
