...
The default mode for `as_duckdb_tibble()` and `duckdb_tibble()` is non_inert.
...
Use `as_duckdb_tibble(inert = "closed")` or `duckdb_tibble(.inert = "closed")` to avoid materializing intermediate data and to ensure that all operations are carried out by DuckDB or fail.
Inertia can also limit the number of rows or cells that are materialized:

```{r}
data <- duckplyr::duckdb_tibble(x = 1:5, .inert = c(rows = 3))
data
```
...
Learn more about inerting in `vignette("inert")`, and about the translation employed by duckplyr in `vignette("limits")`.
