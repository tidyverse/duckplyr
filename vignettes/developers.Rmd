...
Use `as_duckdb_tibble(collect = "always_manual")` or `duckdb_tibble(.collect = "always_manual")` to avoid materializing intermediate data and to ensure that all operations are carried out by DuckDB or fail.
Prudence can also limit the number of rows or cells that are materialized:

```{r}
data <- duckplyr::duckdb_tibble(x = 1:5, .collect = c(rows = 3))
data
```
...
Learn more about prudence in `vignette("collect")`, and about the translation employed by duckplyr in `vignette("limits")`.
