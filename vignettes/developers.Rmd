...
```{r}
untethered$mean_inflight_delay
```
...
The default mode for `as_duckdb_tibble()` is untethered.
...
Use `tether = TRUE` to ensure that all operations are carried out by DuckDB, or fail.
This is also the default for the ingestion functions such as `read_parquet_duckdb()`.

```{r}
tethered <-
  duckplyr::flights_df() |>
  duckplyr::as_duckdb_tibble(tether = TRUE)
```
