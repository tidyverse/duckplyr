...
flights_funneled <-
  flights |>
  duckplyr::as_duckdb_tibble(funnel = "always_manual")
```
...
```{r}
flights_funneled |>
  duckplyr::as_duckdb_tibble(funnel = "automatic") |>
  group_by(origin) |>
  summarize(n = n()) |>
  ungroup()
```
...
A funneled duckplyr frame can be converted to an unfunneled one with `as_duckdb_tibble(funnel = "automatic")`.
...
However, funneled duckplyr frames can be unfunneled at any time, and vice versa.
