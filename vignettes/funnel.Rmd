...
This is explained in the following example that computes the mean arrival delay for warm months:
...
system.time(
  mean_arr_delay_warm <- 
    flights_duckdb |> 
    filter(month >= 3, month <= 10, !is.na(arr_delay)) |> 
    summarize(mean_arr_delay = mean(arr_delay))
)
```

Setting up the pipeline takes almost no time, as the computation is deferred.
This allos DuckDB to optimize the whole pipeline, which can be seen in the output below:
...
The first step is to prune the unneeded columns, only `month` and `arr_delay` are kept.
The result becomes available when accessed, this is where most of the time is spent:

```{r}
system.time(mean_arr_delay_warm$mean_arr_delay[[1]])
```

This is similar to lazy tables in dbplyr and dtplyr,
but different from dplyr where each intermediate step is computed.
