---
title: "Memory protection: Inertia"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{10 Memory protection: Inertia}
...
This vignette discusses eager and lazy computation, and inerting.
...
For non_inert duckplyr frames, as in the two previous examples the underlying DuckDB computation is carried out upon the first request.
...
For inert duckplyr frames, accessing a column or requesting the number of rows triggers an error.
...
The example below demonstrates the use of inert duckplyr frames.

```{r}
flights_inert <-
  flights |>
  duckplyr::as_duckdb_tibble(inert = "always")
```

In this example, `flights_inert` is an inert duckplyr frame.
The data can be displayed, and column names and types can be accessed.

```{r}
flights_inert
names(flights_inert)[1:10]
class(flights_inert)
class(flights_inert[[1]])
```
...
Therefore, inert frames allow you to check that all operations are supported by DuckDB: for an inert frame, fallbacks to dplyr are not possible.
...
The same pipeline with an non_inert frame works, but the computation is carried out by dplyr:

```{r}
flights_inert |>
  duckplyr::as_duckdb_tibble(inert = "never") |>
  group_by(origin) |>
  summarize(n = n()) |>
  ungroup()
```
...
Inert duckplyr frames behave like lazy tables in dbplyr and lazy frames in dtplyr: the computation only starts when you *explicitly* request it with `collect.duckplyr_df()` or through other means.
However, inert duckplyr frames can be non_inert at any time, and vice versa.
In dtplyr and dbplyr, there are no non_inert frames: collection always needs to be explicit.


## Partial inerting

Partial inerting is a compromise between inerting and uninerting.
...
flights_partial <-
  flights |>
  duckplyr::as_duckdb_tibble(inert = c(rows = 100000))
```
...
Partial inerting is a good choice for data sets where the cost of materializing the data is prohibitive only for large results.
