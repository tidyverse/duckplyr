...
flights_frugal <-
  flights |>
  duckplyr::as_duckdb_tibble(collect = "always_manual")
```
...
The same pipeline with an lavish frame works, but the computation is carried out by dplyr:

```{r}
flights_frugal |>
  duckplyr::as_duckdb_tibble(collect = "automatic") |>
  group_by(origin) |>
  summarize(n = n()) |>
  ungroup()
```
...
A frugal duckplyr frame can be converted to an lavish one with `as_duckdb_tibble(collect = "automatic")`.
...
However, frugal duckplyr frames can be lavish at any time, and vice versa.
In dtplyr and dbplyr, there are no lavish frames: collection always needs to be explicit.


## Partial prudence

Partial prudence is a compromise between prudence and unprudence.
...
flights_partial <-
  flights |>
  duckplyr::as_duckdb_tibble(collect = c(rows = 100000))
```
...
Partial prudence is a good choice for data sets where the cost of materializing the data is prohibitive only for large results.
