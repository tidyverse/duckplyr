...
Note that because duckplyr relies heavily on ALTREP, string columns will still be deferred in memory even after `collect()` is called, making `collect()` extremely fast but pushing the internal materialization cost to the first time the data is accessed (for example, when calling `str()`).
