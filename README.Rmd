...
1. By calling `library(duckplyr)`, it is enabled for the entire session. To turn this off, call `methods_restore()`.
...
Use `library(duckplyr)` or `duckplyr::methods_overwrite()` to enable processing with duckdb for all data frames:
